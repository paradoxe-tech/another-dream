...
export function padLevel(data: Map4, shape: number[] = levelShape) {
  ...
  const paddedData: Map4 = [...data, ...worldPadding].map((world: Map3) => {
    ...
    let layerPadding = [];
    if (layerPaddingSize > 0) layerPadding = new Array(layerPaddingSize).fill([]);
    return [...world, ...layerPadding].map((layer: State[][]) => {
      ...
      return [...colPaddingL, ...layer, ...colPaddingR].map((row: State[]) => {
        ...
        let rowPaddingR = [];
        if (rowPaddingSize > 0) {
            rowPaddingL = new Array(Math.floor(rowPaddingSize / 2)).fill(State.Void);
            rowPaddingR = new Array(Math.ceil(rowPaddingSize / 2)).fill(State.Void);
        }
